...
//		String sql="select * from t_dormBuild,t_dormManager,t_connection where dormManId=managerId and dormBuildId=buildId and dormBuildId=?";
	...
	public int managerUpdateWithId (Connection con, String dormManagerId, String dormBuildId, String dormBuildName)throws Exception {
		String sql = "update t_dormManager set dormBuildId=?,dormBuildName=? where dormManId=?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setString(1, dormBuildId);
		pstmt.setString(2, dormBuildName);
		pstmt.setString(3, dormManagerId);
		return pstmt.executeUpdate();
	}
}
