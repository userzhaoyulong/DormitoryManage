...
import com.yulong.dao.DormBuildDao;
import com.yulong.dao.RecordDao;
import com.yulong.dao.StudentDao;
import com.yulong.model.DormManager;
import com.yulong.model.Record;
import com.yulong.model.Student;
...
public class RecordServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("preSave".equals(action)) {
			...
			if(StringUtil.isNotEmpty(startDate)) {
				...
				System.out.println(session.getAttribute("起始日期是:"+"startDate"));
			} else {
				System.out.println(session.getAttribute("移除前起始日期是:"+"startDate"));
	...
	private void recordSave(HttpServletRequest request,
		...
		try {
			...
			DormManager manager = (DormManager)(session.getAttribute("currentUser"));
			int buildId = manager.getDormBuildId();
			String dormBuildName = manager.getDormBuildName();
			System.out.println(dormBuildName);
			Student student = StudentDao.getNameById(con, studentNumber, buildId);
			System.out.println("执行保存缺勤记录前。。。");
			if(student.getName() == null) {
				...
				record.setDormBuildName(dormBuildName);
				record.setDormName(student.getDormName());
				record.setStartDate("2018-1-2");
				record.setEndDate("2018-1-3");
				if(StringUtil.isNotEmpty(recordId) && Integer.parseInt(recordId)!=0) {
					saveNum = recordDao.recordUpdate(con, record);
				} else {
					System.out.println("将要存储的缺勤记录信息:"+"studentNumber:"+record.getStudentNumber()+"studentName:"+"date:"+record.getDate()+"detail:"+record.getDetail()+"dormBuildId:"+record.getDormBuildId()+"dormBuildName:"+record.getDormBuildName()+"dormName:"+record.getDormName()+"startDate:"+record.getStartDate()+"endDate:"+record.getEndDate());
